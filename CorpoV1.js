...
export async function main(ns) {
  ...
  ns.ui.openTail();
    ...
    if (corp.divisions.length < 2 && corp.numShares == corp.totalShares) {
			if (ns.corporation.getDivision("Tobacco").products.length > 2) {
				await trickInvest(ns, corp.divisions[0]);
        ns.corporation.getDivision("Tobacco").products.length
			}
		}
		await ns.sleep(5000);
	}
...
async function hireEmployees(ns, division, productCity = "Sector-12") {
	let employees = ns.corporation.getOffice(division, productCity).numEmployees;
...
function upgradeWarehouses(ns, division) {
    ...
    let wareHouseExists = ns.corporation.hasWarehouse(division, city);
    if (!wareHouseExists) continue;
    let cityWarehouse = ns.corporation.getWarehouse(division, city);
...
function newProduct(ns, division) {
  let productNumbers = ns.corporation.getDivision(division).products;
  let products = ns.corporation.getDivision(division).products || [];
  ns.print(ns.corporation.getDivision(division).products.length);
  ns.print(productNumbers);

  for (let prodObj of products) {
    if (!prodObj || !prodObj.name) continue;
    const product = prodObj.name;

    let prodInfo = ns.corporation.getProduct(division, "Sector-12", product);

    if (prodInfo.developmentProgress < 100) {
      ns.print(division + " Product development progress: " + prodInfo.developmentProgress.toFixed(1) + "%");
      return;
    } 
    else {
      if (prodInfo.sCost == 0) {
...
*/
  ...
  if (productNumbers.length < numProducts) {
    ns.print("Start new product development ", "Product-" + Date.now());
	  ns.corporation.makeProduct(division, "Sector-12", "Product-" + Date.now(), productInvest, productInvest);
  }
}
