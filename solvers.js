...
solvers["Compression III: LZ Compression"] = (data) => {
    ...
    function set(state, i, j, str) {
        ...
        if (current == null || str.length < current.length) {
            ...
            // randomness improves compatibility with Compression II
            state[i][j] = str;
        }
    }
    ...
    for (let i = 1; i < plain.length; ++i) {
        ...
        for (let offset = 1; offset <= 9; ++offset) {
            for (let length = 1; length <= 9; ++length) {
                ...
                // end backreference and start a new one
...
solvers["Square Root"] = (data) => {
    // Ensure BigInt (Bitburner usually gives this as a number or string)
    const n = BigInt(data);

    const two = 2n;
    if (n < two) return n.toString(); // sqrt(0)=0, sqrt(1)=1
    ...
    // Final sanity check (should never fail)
    ...
    if (
        ...
        throw new Error(
            `Square Root did not converge. root=${root}, root^2=${root * root}, n=${n}`
        );
    }
...
solvers["Encryption I: Caesar Cipher"] = ([text, shift]) => {
...
solvers["Encryption II: Vigenère Cipher"] = ([text, key]) => {
    const A = "A".charCodeAt(0);
    ...
    for (const c of text) {
        ...
        if (code >= 65 && code <= 90) {
            ...
            res += String.fromCharCode(decoded);
            j++;
        } else {
            res += c;
        }
